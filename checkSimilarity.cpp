...
void checkAllParticleSimilarity(Particle *particle){
    ...
    int haveRun=0;
    while (haveRun < particle_numbers){
        for (int i=0; i<tidSize && haveRun+i<particle_numbers; i++){
...
void *checkOneParticleSimilarity(void *p) {
    ...
    printPdb(particle);
    runTM_score(particle->my_Index);
    double TM_score = getTM_score(particle->my_Index);
...
void becomeInitialParticle(Particle &particle) {
    inputParticle(particle, particle.my_Index, particle.seq);
}

/*double getTM_score(int my_Index){
    const char *scoreAnswer = catStrIntStr(tempFileAddress, "score", my_Index, ".txt");
